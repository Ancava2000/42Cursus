...
void ft_list_remove_if(t_list **begin_list, void *data_ref, int (*cmp)())
{
    ...
    if ((*begin_list == NULL) || (begin_list == NULL))
        return ;
    if(cmp((*begin_list) -> data, data_ref) == 0)  // it frees the node if the function returns 0 (when both parameters are equal, ==)
    {
        ...
        *begin_list = (*begin_list) -> next;       // the new first node in the list is now the second
        ...
        ft_list_remove_if(&((*begin_list) -> next), data_ref, cmp);   // IMPORTANT, if there aren't equal we have to continue with the next node (&)
}
