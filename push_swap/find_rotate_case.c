...
/*   Updated: 2024/10/09 11:50:47 by acarro-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

/* This function finds the adecuate rotation combination
with the less number of moves.
Within a loop, it compares the variable i
(with the number of moves of case rrarrb_b),
with each case. If the original number is bigger,
the variable stores the number of moves of the new case,
until it gets the smallest number.
In the last step the temp variable starts the loop with the next node.
In each case, after the rotations it always pushes the node.
In this case, it pushes the node from
stack "a" to stack "b" */
